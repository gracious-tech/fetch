...
p But there are #[a(href='https://copy.church/initiatives/bibles/' target='_blank') many things you #[strong can't] do] with such bibles (without special permission) due to copyright, such as print portions of them yourself, include them in ministry resources, distribute audio recordings of them, etc.
...
// Detect modern years
const modern_years = new Date().getFullYear() - collection.bibles._modern_year
...
const languages_with_shareable = collection.bibles.get_languages().map(item => item.code).filter(lang => {
    ...
    }).some(t => t.year >= collection.bibles._modern_year)
})
